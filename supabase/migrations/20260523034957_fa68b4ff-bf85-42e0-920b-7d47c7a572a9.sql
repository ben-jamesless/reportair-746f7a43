-- 1. Free-tier update-day counter
CREATE OR REPLACE FUNCTION public.get_project_update_day_count(_project_id uuid)
RETURNS integer
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COUNT(DISTINCT DATE(created_at AT TIME ZONE 'UTC'))::integer
  FROM public.photos
  WHERE project_id = _project_id;
$$;
GRANT EXECUTE ON FUNCTION public.get_project_update_day_count(uuid) TO authenticated;

-- 2. Allow 'free' plan in admin setter
...
BEGIN
  ...
  END IF;
  IF _plan NOT IN ('free', 'solo', 'pro', 'studio') THEN
    RAISE EXCEPTION 'Invalid plan: must be free, solo, pro, or studio';
  END IF;
  UPDATE public.teams SET plan = _plan WHERE id = _team_id;
END $function$;

-- 3. Extend resolve_share_link to expose team plan + logo + name for share-page branding
...
BEGIN
  ...
  SELECT t.plan, t.name AS team_name, t.logo_path
    INTO team_row
