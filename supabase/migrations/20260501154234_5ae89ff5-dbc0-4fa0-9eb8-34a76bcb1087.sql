-- Extend resolve_share_link to include latest ready export and ensure project includes branding/status fields
...
BEGIN
  ...
  SELECT id, name, description, template, client_name, event_type, event_location, event_date, color, overall_status
...
-- Public RPC to get a signed download URL for the latest ready export of a share link
CREATE OR REPLACE FUNCTION public.get_share_export_url(_token uuid)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
